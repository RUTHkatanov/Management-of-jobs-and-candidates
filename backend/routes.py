...
def create_task(request: Request, task: Task = Body(...)):
    ...
    created_task = database["tasks"].find_one(
...
def find_task(id: str, request: Request):
    if (task := database["tasks"].find_one({"_id": id})) is not None:
        return task
...
def update_task(id: str, request: Request, task: TaskUpdate = Body(...)):
    ...
    if len(task) >= 1:
        update_result = database["tasks"].update_one(
            {"_id": id}, {"$set": task}
        )
    ...
    if (
        existing_task := database["tasks"].find_one({"_id": id})
    ) is not None:
...
def delete_task(id: str, request: Request, response: Response):
    delete_result = database["tasks"].delete_one({"_id": id})
